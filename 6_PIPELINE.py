...
[cm_1_PL, sensi_1_PL, spezi_1_PL, richtigkl_1_PL, 
 y_pred_1_PL, mydf_1_PL] = RFAll(dtm_train_klein, dtm_test_klein, y_train_klein, y_test_klein, X_test_ind,
...
b_words = ['alt arbeitgeb', 'aufheb vertrag', 'beschaftigt', 'betrieb', 'mitarbeiterin']

r_words = ['altersrent', 'mehr arbeitsfah', 'regelaltersrent', 'rent', 'rentenbeginn', 'ruhestand']
 
t_words = ['gestorb', 'verstorb']


GrList = []
for i in range(len(X_test_K_preds)):
    dok_K = []
    dok = X_test_K_preds[i].split()
    ...
    if any(map(lambda v: v in b_words, dok)):
        dok_K += 'B'
    elif any(map(lambda v: v in b_words, dok_bigram)):
        dok_K += 'B'
    ...
    if any(map(lambda v: v in t_words, dok)):
        dok_K += 'T'
    elif any(map(lambda v: v in t_words, dok_bigram)):
        dok_K += 'T'
    if dok_K == []:
        dok_K = ['K']
